...
async function callProvider({ provider, apiKey, model, images, base64, mime, prompt }) {
  ...
  return fn({ apiKey, model, images, base64, mime, prompt });
}
...
async function chatParseFood({ provider, apiKey, model, history, images, base64, mime }) {
  const imgList = normalizeImages({ images, base64, mime });
  const prompt = `${CHAT_PROMPT}\n\n${formatChatHistory(history)}Обрабатывай последнее сообщение пользователя из истории выше${imgList.length > 0 ? ` (к нему приложено ${imgList.length} фото)` : ''}.`;
  const result = await callProvider({ provider, apiKey, model, images: imgList, prompt });
  return {
